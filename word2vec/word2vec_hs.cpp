...
void word2vec_hs::init(long long V, long long D, const int *freq_table) {
    ...
    _W1_momentum = alloc_2d(V, D, aio_zero);
    _W2 = alloc_2d(V - 1, D, aio_random);
    _W2_momentum = alloc_2d(V - 1, D, aio_zero);
    _e = 0.01f;
    _g = 0.9f;
...
float word2vec_hs::step(const word_io_pair *trains, long long ccExample) {
    if (!_is_init) {
    ...
    for (int _i = 0; _i < ccExample; _i++) {
        // forward propagation
        long long ini = trains[_i].in;
        for (int j = 0; j < _D; j++) {
            hidden_layer[j] = _W1[ini][j];
        }
        memset(W1_update, 0, sizeof(float) * _D);
        long long cur_node = _node_tree[trains[_i].out];
        bool side = _parent_side[trains[_i].out];
        do {
            long long outi = cur_node - _V;
            // forward propagation
            float innerp = 0;
            for (int j = 0; j < _D; j++) innerp += _W2[outi][j] * hidden_layer[j];
            float sign = side ? 1.0f : -1.0f;
            float a = sigmoid(sign * innerp);
            loss += -logf(a + 0.0000000000001f);
            // update W1 layer
            for (int j = 0; j < _D; j++) {
                W1_update[j] -= _e * sign * (-_W2[outi][j] * (1 - a));
            }
            // update HS layer
            for (int j = 0; j < _D; j++) _W2[outi][j] -= _e * sign * (-hidden_layer[j] * (1 - a));
            side = _parent_side[cur_node];
            cur_node = _node_tree[cur_node];
        } while (cur_node != -1);
        for (int j = 0; j < _D; j++) {
            _W1[ini][j] += W1_update[j];
        }
    }
...
float word2vec_hs::step_skip_gram(wordmgr &wm, const int *trains, long long ccTrain, int thread_id) {
    ...
    for (int _i = 0; _i < ccTrain; _i++) {
        ...
        //long long window_sz = (random() % 5) + 1;
...
float word2vec_hs::step_skip_gram_sgd(wordmgr &wm, long long ccTrain) {
    if (!_is_init) {
        throw std::runtime_error(u8"word2vec_hs: step() cannot be called before init");
    }
    _rand_state = tick64();
    float *hidden_layer = new float[_D];
    float *W1_update = new float[_D];
    float **W2_update = alloc_2d(_V - 1, _D, aio_zero);
    float loss = 0;

    for (int _i = 0; _i < ccTrain; _i++) {
        // training example generation
        long long in_idx = random() % wm.corpus_word_count();
        long long in = wm._src_words[in_idx];
        //long long window_sz = (random() % 5) + 1;
        long long window_sz = 6;

        // forward propagation
        for (int j = 0; j < _D; j++) {
            hidden_layer[j] = _W1[in][j];
        }
        memset(W1_update, 0, sizeof(float) * _D);
    ...
    add_batch(_W2[0], W2_update[0], (_V - 1)*_D);
    delete[] hidden_layer;
    delete[] W1_update;
    free_2d(W2_update);
    return loss / ccTrain;
...
float word2vec_hs::step_momentum(const word_io_pair *trains, long long ccExample) {
    if (!_is_init) {
        throw std::runtime_error(u8"word2vec_hs: step() cannot be called before init");
    }

    float *hidden_layer = new float[_D];
    float *W1_grad = new float[_D];
    float *tmp = new float[_D + 64];
    float loss = 0;

    for (int _i = 0; _i < ccExample; _i++) {
        // forward propagation
        long long ini = trains[_i].in;
        memcpy(hidden_layer, _W1[ini], sizeof(float) * _D);
        //for (int j = 0; j < _D; j++) {
        //    hidden_layer[j] = _W1[ini][j];
        //}
        memset(W1_grad, 0, sizeof(float) * _D);
        long long cur_node = _node_tree[trains[_i].out];
        bool side = _parent_side[trains[_i].out];
        do {
            long long outi = cur_node - _V;
            ...
            // update W1 layer
            for (int j = 0; j < _D; j++) {
                W1_grad[j] += sign * (-_W2[outi][j] * (1 - a));
            }
            // update HS layer
            for (int j = 0; j < _D; j++) {
                float grad = _e * sign * (-hidden_layer[j] * (1 - a));
                _W2_momentum[outi][j] = _g * (_W2_momentum[outi][j]) - _e * (grad);
                _W2[outi][j] += _W2_momentum[outi][j];
            }
            side = _parent_side[cur_node];
            cur_node = _node_tree[cur_node];
        } while (cur_node != -1);
        for (int j = 0; j < _D; j++) {
            _W1_momentum[ini][j] = _g * (_W1_momentum[ini][j]) - _e * (W1_grad[j]);
            _W1[ini][j] += _W1_momentum[ini][j];
        }
    }
